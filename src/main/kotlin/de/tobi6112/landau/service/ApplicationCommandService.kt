...
class ApplicationCommandService(
  ...
  fun createCommands(commands: Iterable<Command>, config: CommandsConfig) {
    ...
    val globalEnabledCommands = commands
        .filter { config.global[it.name]?.enabled ?: true }
    createGlobalCommands(globalEnabledCommands)

    // Get guild enabled commands
    val guildEnabledCommands = config.guilds
        .mapValues { (guildId, cmdCfg) ->
          commands
              .filter { cmdCfg[it.name]?.enabled ?: true }
        }
    guildEnabledCommands
        .forEach { createGuildCommands(it.key, it.value) }
  }
  ...
  private fun createGlobalCommands(commands: Iterable<Command>) {
    ...
    commands.distinctBy { it.name }.forEach {
      ...
      this.applicationService.createGlobalApplicationCommand(applicationId, request)
  ...
  private fun createGuildCommands(guildId: Long, commands: Iterable<Command>) {
    ...
    commands.distinctBy { it.name }.forEach {
      val request = commandToApplicationCommandRequest(it)
      this.applicationService.createGuildApplicationCommand(applicationId, guildId, request)
          .doOnError { err -> logger.error(err) { "Could not create guild command $it on guild $guildId" } }
          .doOnSuccess { logger.info { "Successfully created global command $it on guild $guildId" } }
          .subscribe()
    }
  }

  private fun checkMultipleCommands(commands: Iterable<Command>) {
    commands.groupingBy { it.name }
        .eachCount().filter { it.value > 1 }
        .forEach { (name, count) -> logger.warn { "Multiple commands for name $name found ($count times)" } }
  }

  private fun commandToApplicationCommandRequest(command: Command) = ApplicationCommandRequest.builder()
      .name(command.name)
